...
class CloneLastFMArtists extends Command
{
    ...
    public function handle(LastFMService $lastFM): int
    {
        ...
        $this->info(">>> FETCHING ARTISTS");
        // Artists
        $this->fetchArtists();


        $this->info(">>> FETCHING TRACKS");
        // Tracks
        $this->fetchTracks();
     ...
     */
    protected function fetchTracks(): void
    {
        $artists = Artist::all();
    ...
    public function addArtistsToDatabase($artists): void
    {
        ...
        foreach ($artists as $artist) {
            if (Artist::where('last_fm_url', $artist->url)->exists()) {
                $this->warn("Not adding duplicated artist \"$artist->name\"");
                continue;
            }
            $this->info("Adding artist \"$artist->name\" to database");
    ...
    public function addTracksToDatabase($trackPage, $artist): void
    {
        foreach ($trackPage as $track) {
            try {
                ...
                $this->info("$artist - $track->name added to database");
